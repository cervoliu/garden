...
与[[Alive2 - Bounded Translation Validation for LLVM]] 的一个不同点在于，Alive2 会先根据 LLVM bitcode 生成 CFG （Triton 是不会生成 CFG 的），然后在 CFG 上合并执行路径，将语义编码为 ite 语句嵌套表示的公式。因此，Alive2 的一个函数分析结束后对应一个大公式，其综合了 bound 之内所有执行路径的语义。
