...
1. **引入路径集（Pathsets）单步对齐，解决指数级爆炸**： 与以往（如 Dahiya 工作）逐条匹配单一程序路径不同，Counter 算法引入了“路径集”的概念。它使用**串并联有向图（series-parallel digraph）**来表示包含指数级数量路径的路径集，从而可以在单步中直接对原程序和优化后程序的一整组路径进行关联，极大地控制了 SMT 求解的复杂度（保持在线性规模）。
2. **放宽匹配条件为“单向蕴含”（One-way Implication）**： 摒弃了 Dahiya 等人要求两条关联路径的控制流条件必须“完全等价”的苛刻限制。Counter 算法的对齐标准是：如果汇编程序执行了其路径集中的某条路径，那么源程序**必然**会执行其对应路径集中的某条路径。这使得算法能够稳健地处理循环拆分、条件外提等会导致路径特化的复杂编译器优化。
3. **反例引导的最佳优先搜索（Counterexample-Guided Best-First Search）**： 放弃了 SPA（Churchill 等人）对用户提供的高覆盖率动态测例以及需要人工“猜测”对齐谓词（Alignment Predicate）的强依赖。算法在增量构建乘积程序的每一步，利用 SMT 求解器生成的具体**反例（Counterexamples，即满足当前不变式的具体机器状态）**来动态引导搜索。
4. **动态修剪（Pruning）与排序（Ranking）机制**：
    - **修剪**：如果在当前的反例状态下，机器走通了汇编程序的候选路径，却无法走通源程序对应的任何候选路径，或者两者的堆内存关系不再匹配，算法会立刻修剪掉这个错误的关联分支。
    - **排序**：算法对反例集计算最强不变式覆盖，并统计**在仿射关系（Affine relations）中相互关联的活跃位向量变量的数量**。关联的变量越多，说明该对齐方式越有可能是正确的，算法会赋予其更高的优先级（Ranking）。通过这种机制，算法能以极低的代价在庞大的搜索空间中迅速收敛到正确的乘积程序图（Product-CFG）。