...
feishu_shared_at: "2026-02-28 22:03"
---
## 概要
...
有别于以往基于语法的对齐技术，本文提出了一种语义驱动的程序对齐技术。该技术通过用户提供的一组测例，配合启发式的对齐谓词，构建起（定义在程序状态上的，语义级的）轨迹对齐（trace alignment），并借此进一步构建（定义在程序位置上的，语法级的）程序对齐自动机（PAA，其唯一确定一个乘积程序）。
...
轨迹对齐可以诱导出**对应路径**（corresponding paths），如 Figure 5。
...
用程序对齐自动机（Program Alignment Automaton，PAA）表示附有对齐信息的乘积程序。PAA 的一个结点 $s$ 处附带一组程序位置 $(u,v)$ 以及 $s$ 处的不变式 $\phi_s$。PAA 的一条转移边 $(u,u') \to (v,v')$ 附带一组有限的程序路径 $(P,Q)$，其中 $P$ 为 $f$ 的一条从 $u$ 到 $v$ 的路径，$Q$ 为 $g$ 的一条从 $u'$ 到 $v'$ 的路径，且 $P,Q$ 不能均为空。
...
用 PAA 证明程序等价性：
1. 对于每条转移边 $s \xrightarrow{P, Q} t$，证明 $\hoare{\phi_s}{P;Q}{\phi_t}$（i.e. inductivity of transition）。
2. 对于每个结点 $s = (u,u')$ ，所有不在 PAA 中的从 $(u,u')$ 出发的程序路径对均不可达（即，PAA 没有遗漏乘积程序语义，是一个 sound 的上近似）。形式化地，若 $P, Q$ 分别为 $f, g$ 中从 $u, u'$ 出发的路径，且 PAA 不存在转移 $s \xrightarrow{P^{*},Q^{*}} s'$ 满足 $P^{*},Q^{*}$ 分别为 $P,Q$ 的前缀，则有 $\hoare{\phi_{s}}{P;Q}{\bot}$ 。
3. PAA 中不存在由转移边构成的回路，使得回路的每条转移边上 $f$ 侧（或 $g$ 侧）的程序路径均为空（否则，$g$ （或 $f$）非终止）。
4. 出口结点处的不变式蕴含堆内存状态以及输出寄存器相等。
