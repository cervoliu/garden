...
值得注意的是，$\psi$ 在求解是为 hard constraints，而后面的一系列 boundary constraints 为 soft constraint。


上述求解得到的 unsat core 即使在 size 上是 minimal 的，其对应的区间也不一定是最 general 的。
考虑如下的 **digging generalization**:
首先定义 $\mathcal{I}_{x,\mu_{x},k}$ 是 $2m+1$ 个相邻区间，表示了 $x$ 的 domain: 
...
$$ \psi \land \bigwedge_{x \in \vec{x}} \bigwedge \{ \neg \mathcal{I}_{x,\mu_{x},k} \mid k=-\infty,-m,\dots,-1,1,\dots,m,\infty \} $$
	依旧 $\psi$ 是 hard constraint，其他项是 soft constraints。对于这个查询求出的 unsat core $\{\mathcal{I}_1, \dots, \mathcal{I}_{m}\}$，$\wedge_{i=1}^{m} \neg \mathcal{I}_{i}$ 表示了若干个不相交的可以作为 counterexample 的 x 的取值区间。取包含 $\mu$ 的那个区间。

---

区间泛化技术可以在 [[Triton-based Equivalence Checker]] 中使用吗？

- 对于 QF_ABV 理论下的公式，如何定义区间？boundary constraints 是什么样的？
